...
class ExtendedKalmanFilter:
    def __init__(
        ...
        self.chi_thresh = 7.815
        self.lambda_min = 0.9
        self.lambda_scale = 0.005

    def set_adaptive_parameters(self, chi_thresh=7.815, lambda_min=0.9, lambda_scale=0.005):
        self.chi_thresh = chi_thresh
    ...
    def update(self, z):
        ...
        # --- Phase 2: RVFF-KF (Forgetting Factor based on Mahalanobis Distance) ---
        eps_sq = innovation.T @ s_inv @ innovation
        
        if eps_sq > self.chi_thresh:
            # We scale self.p by 1/lambda_k to forget past state information
            lambda_k = max(self.lambda_min, 1.0 - self.lambda_scale * (eps_sq - self.chi_thresh))
            self.p = (1.0 / lambda_k) * self.p
