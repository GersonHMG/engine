...
local START_GOAL = { x = 0.0, y = -3.0 }  -- Arco de origen (sur)
local TARGET_GOAL = { x = 0.0, y = 3.0 }  -- Arco objetivo (norte)
...
grsim.teleport_robot(robotId, team, 0.0, -3.3, math.pi / 2)
...
function m.process()
    ...
    draw_line({ { x = 0.0, y = -3.0 }, { x = 0.0, y = 3.0 } }, true, { r = 0.0, g = 0.8, b = 1.0 })
    ...
    if robot then
        draw_text(robot.x + 0.2, robot.y, "Prueba Patada Eje Y (Goal to Goal)", { r = 1.0, g = 1.0, b = 0.0 })
    end
