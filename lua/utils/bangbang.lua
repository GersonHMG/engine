-- ============================================================================
-- Módulo de Control Bang-Bang para Trayectorias Omnidireccionales
-- ============================================================================
...
BangBang = {}
BangBang.__index = BangBang

-- ----------------------------------------------------------------------------
-- Funciones Matemáticas Locales (Privadas del módulo)
-- ----------------------------------------------------------------------------
...
local function get_trajectory_2d_velocity(a_max, v_max, vx0, vy0, from_x, from_y, to_x, to_y)
...
--- Crea un nuevo controlador con límites físicos dados
function BangBang.new(a_max, v_max)
    return setmetatable({ a_max = a_max, v_max = v_max }, BangBang)
end

function BangBang:is_near_to_brake(rx, ry, vx, vy, target_x, target_y)
...
--- Computa el comando de movimiento hacia una lista de waypoints (path)
function BangBang:compute_motion(robot, path, delta)
    ...
    local global_vx, global_vy = get_trajectory_2d_velocity(
    ...
    -- Transformación a sistema local del robot
...
-- Exportamos la tabla con las funciones públicas
return BangBang